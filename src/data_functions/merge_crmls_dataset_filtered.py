...
"""Build and update separate CRMLS listing/sold master CSV datasets.

Workflow:
1. Read historical monthly files from raw/.
2. Read the newest monthly CSVs generated in the project root.
3. Keep listing and sold as two separate master datasets.
4. When a month is regenerated, replace that month in the master file instead of duplicating it.
"""
...
# First build uses historical files from raw/ plus any newly generated monthly files in the project root.
BOOTSTRAP_DIRS = ("raw", ".")
# Later runs only need to look for the latest monthly files in the project root.
DEFAULT_UPDATE_DIRS = (".",)
# These metadata columns are added to the output so each row keeps its source month/file.
METADATA_COLUMNS = ("source_file", "file_period", "sort_date")
# The historical window starts in January 2024.
START_PERIOD = "202401"
MASTER_FILES = {
...
def normalize_headers(headers: list[str]) -> list[str]:
    """Make duplicate CSV headers unique so they can be stored safely in dictionaries."""
...
def resolve_dir(project_root: Path, directory: str) -> Path:
    """Resolve a source directory relative to the project root."""
    path = Path(directory)
...
def unique_directories(directories: tuple[str, ...]) -> tuple[str, ...]:
    """Keep directory order stable while removing duplicates."""
    ordered: list[str] = []
    seen: set[str] = set()
...
def most_recent_completed_period() -> str:
    """Return YYYYMM for the latest fully completed calendar month."""
...
def collect_source_files(project_root: Path, directories: tuple[str, ...], end_period: str) -> list[SourceFile]:
    """Collect one source file per dataset type and month, using later directories as higher priority.

    Example:
    - raw/CRMLSListing202603.csv exists
    - ./CRMLSListing202603.csv is newly generated this month
    The project-root file wins and replaces that month in the master dataset.
    """
...
def read_csv_rows(csv_path: Path) -> tuple[list[str], list[dict[str, str]]]:
    """Read a CSV into rows while preserving all columns, including duplicated headers."""
...
def read_master(master_path: Path) -> tuple[list[str], list[dict[str, str]]]:
    """Read an existing master dataset if it already exists."""
...
def derive_sort_date(row: dict[str, str], dataset_type: str, file_period: str) -> str:
    """Choose the best available date column so merged rows stay in chronological order."""
...
def merge_header_order(existing_headers: list[str], new_headers: list[str]) -> list[str]:
    """Keep metadata columns first, then preserve the existing column order as much as possible."""
...
def row_sort_key(row: dict[str, str]) -> tuple[str, str, str, str]:
    """Sort by source month first, then by the best available row-level date."""
    return (
        row.get("file_period", ""),
        row.get("sort_date", ""),
...
def write_master(master_path: Path, headers: list[str], rows: list[dict[str, str]]) -> None:
    """Write the final combined master CSV for listing or sold."""
...
def build_incoming_rows(
    ...
    """Load all incoming monthly rows for one dataset type and attach metadata columns."""
...
def filter_residential_only(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    """Keep only residential records, matching the analysis requirement."""
...
def update_master_for_type(master_path: Path, dataset_type: str, sources: list[SourceFile]) -> None:
    """Merge monthly updates into one master file for either Listing or Sold.

    If a month is present in the new incoming files, the old rows for that same month
    are removed before the replacement rows are written back out.
    """
...
def masters_exist(project_root: Path) -> bool:
    """Check whether both master datasets have already been created."""
    return all((project_root / master_file).exists() for master_file in MASTER_FILES.values())


def parse_args() -> argparse.Namespace:
    """Parse command-line options for normal update mode or a full rebuild."""
    parser = argparse.ArgumentParser(
        description=(
            "Initialize or update separate CRMLS listing/sold master datasets. "
            "First run bootstraps from raw plus any newly generated monthly CSVs in the project root. "
            "Later runs update from the project root only."
        )
    )
    parser.add_argument(
        "--project-root",
    ...
    parser.add_argument(
        "--rebuild",
        action="store_true",
        help="Force a full rebuild from raw plus the project root even if master files already exist.",
    )
    parser.add_argument(
        "--scan-dir",
        action="append",
...
def main() -> None:
    """Choose source folders, collect monthly files, and update both master datasets."""
    args = parse_args()
    ...
    if bootstrap:
        scan_dirs = unique_directories(BOOTSTRAP_DIRS + tuple(args.scan_dir))
        print(f"[mode] bootstrap from: {', '.join(scan_dirs)} ({START_PERIOD} to {end_period})")
    else:
        scan_dirs = unique_directories(DEFAULT_UPDATE_DIRS + tuple(args.scan_dir))
